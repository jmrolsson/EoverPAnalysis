...
if [ $# -eq 0 ]
  ...
  then
    ...
    tag=$1
    today=$(date +"%Y%m%d")
    files_data=EoverP/filelists/data15_13TeV_lowmu_test1.txt
    files_mc=EoverP/filelists/mc15_13TeV_lowmu_test1.txt

    mkdir -p results

    echo "---> Running data:"
    echo xAH_run.py --files ${files_data} --inputList --config EoverP/scripts/config_eop_data.py --submitDir results/condor_test_eop_lowmu_data_${today}_${tag} --verbose --force condor --optFilesPerWorker 1
    xAH_run.py --files ${files_data} --inputList --config EoverP/scripts/config_eop_data.py --submitDir results/condor_test_eop_lowmu_data_${today}_${tag} --verbose --force condor --optFilesPerWorker 1

    echo "---> Running MC:"
    echo xAH_run.py --files ${files_mc} --inputList --config EoverP/scripts/config_eop_mc.py --submitDir results/condor_test_eop_lowmu_mc_${today}_${tag} --verbose --force condor --optFilesPerWorker 1
    xAH_run.py --files ${files_mc} --inputList --config EoverP/scripts/config_eop_mc.py --submitDir results/condor_test_eop_lowmu_mc_${today}_${tag} --verbose --force condor --optFilesPerWorker 1
