...
if [ $# -eq 0 ]
  ...
  then
    ...
    echo "--> Jobs submitted!"
    echo "source $ROOTCOREBIN/../EoverPAnalysis/scripts/merge_condor_eop.py $ROOTCOREBIN/../results/run_condor_eop_lowmu_ND_SD_DD_latest.log # when condor jobs are finished to merge output files"
