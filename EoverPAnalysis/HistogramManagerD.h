#ifndef EoverPAnalysis_HistogramManagerD_H
#define EoverPAnalysis_HistogramManagerD_H

/** @file HistogramManagerD.h
...
/**
    ...
    @rst
        ...
        We expect the user to create a new group of histograms, such as for jets::
            ...
            class JetHists : public HistogramManagerD
            {
              public:
                ...
                StatusCode execute( const xAOD::JetContainer* jets, double eventWeight, int pvLoc = -1);
                StatusCode execute( const xAOD::Jet* jet, double eventWeight, int pvLoc = -1 );
                using HistogramManagerD::book; // make other overloaded version of book() to show up in subclass
...
class HistogramManagerD {
  ...
  public:
    ...
    */
    HistogramManagerD(std::string name, std::string detailStr);
    /**
        @brief Destructor, allows the user to delete histograms that are not being recorded.
    */
    virtual ~HistogramManagerD();

    /**
        @brief Initialize and book all histograms.
        @rst
            .. note:: This should call the overloaded functions :cpp:func:`HistogramManagerD::book` to create the histograms so that the user can call `hists->record(wk())` to record all histograms to the EventLoop worker.
    ...
    /**
        ...
        @rst
            Example implementation::
                ...
                StatusCode JetHists::execute( const xAOD::JetContainer* jets, double eventWeight ){
    ...
    */
     ...
     * @overload
     */
    TH2D* book(std::string name, std::string title,
               std::string xlabel, int xbins, double xlow, double xhigh,
     ...
     * @overload
     */
    TH3D* book(std::string name, std::string title,
               std::string xlabel, int xbins, double xlow, double xhigh,
    ...
    /**
     * @brief record all histograms from HistogramManagerD#m_allHists to the worker
     */
  ...
  private:
    ...
    /**
     * @brief Push the new histogram to HistogramManagerD#m_allHists
     */
